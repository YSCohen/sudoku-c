    ...
    for (int row = 0; row < SUDOKU_SIZE; row++) {
        for (int column = 0; column < SUDOKU_SIZE; column++) {
            ...
            if (value != SUDOKU_EMPTY &&
                !sudoku_is_value_valid(board, row, column, value)) {
                return 0;
            }
        }
    }
...
int sudoku_count_solutions(SudokuBoard *board, int limit) {
    ...
    if (board == NULL || board->cells == NULL ||
        limit < 1 || !sudoku_is_board_valid(board)) {
        return 0;
    }
...
SudokuBoard *sudoku_generate_puzzle(const SudokuBoard *solution,
    ...
    if (solution == NULL || solution->cells == NULL ||
        !sudoku_is_complete(solution) ||
        !sudoku_is_board_valid(solution)) {
        return NULL;
    }
